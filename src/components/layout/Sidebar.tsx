...
export default function Sidebar({ 
  ...
  return (
    ...
    >
      ...
      {!isCollapsed && (
        <div className="px-4 py-3 border-b border-slate-800/50 select-none">
          <div className="flex items-center gap-2 bg-black/20 p-1.5 rounded-xl border border-slate-800/50">
            <span className="text-[9px] font-bold uppercase tracking-wider text-slate-500 pl-2">Role</span>
            <div className="flex-1 flex gap-1">
              <button 
                onClick={() => setActiveRole("Counsel")}
                className={`flex-1 text-center py-1.5 rounded-lg text-xs font-semibold transition-all duration-150 cursor-pointer ${
                  activeRole === "Counsel" 
        ...
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          {!isCollapsed && (
            <div className="flex flex-col min-w-0">
              <span className="text-sm font-semibold text-white truncate leading-none">Sarah Mitchell</span>
              <span className="text-[10px] text-slate-400 font-medium truncate mt-1">Legal Counsel</span>
            </div>
          )}
        </div>
        {!isCollapsed && (
